...
<html>
    ...
    <body class="background_2">
            ...
            <div class="w3-content" style="max-width:1200px">
                <div class="w3-main">
                    ...
                    <header class="w3-container w3-xxxlarge">
                        <p class="w3-center w3-text-white">Ocjena</p>
                    </header>

                    <div class="w3-container w3-light-grey w3-padding-32 m_padding_small" style="text-align: center">
                        <div>
                            <?php
                            if($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET["id"]) && isset($_GET["p"])) {
                                ...
                                if($kod === $_GET["id"]) {
                                    ...
                                    $bp = new Baza();
                                    $sql = "SELECT ocjena FROM ocjena " .
                                            "WHERE kupac_korisnicko_ime = '" . $kupac . "' AND " .
                                            "proizvodjac_korisnicko_ime = '" . $proizvodjac . "'";
                                    $sql2 = "SELECT naziv_opg FROM korisnici " .
                                            "WHERE korisnicko_ime = '" . $proizvodjac . "'";
                                    $bp->spojiDB();
                                    $rs = $bp->selectDB($sql);
                                    if ($bp->pogreskaDB()) {
                                        exit();
                                    ...
                                    while (list($k_opg) = $rs2->fetch_array()) {
                                        $opg = $k_opg;
                                    }
                                    $rs2->close();
                                    if($rs->num_rows === 0) {
                                        echo '<input id="opg" name="opg" type="hidden" value="' . $opg . '">';
                                        echo '<input id="kupac" name="kupac" type="hidden" value="' . $kupac . '">';
                                        echo '<input id="proizvodjac" name="proizvodjac" type="hidden" value="' . $proizvodjac . '">';
                                        echo '<div style="display: inline-block">';
                                            echo '<p style="margin-top: 24px; margin-bottom: 24px">';
                                            echo "Ako želite, možete ocijeniti proizvođača $opg:</p>";
                                        echo "</div>";
                                        echo '<div style="display: inline-block; margin-bottom: 15px">';
                                            echo '<select id="ocjena" name="ocjena" class="w3-select w3-border" style="width: 40px; margin: 15px">';
                                        ...
                                        echo '<div style="display: inline-block; vertical-align: top">';
                                            echo '<button id="ocijeni" name="ocijeni" type="button" class="w3-btn w3-padding w3-red w3-margin-top w3-margin-bottom w3-margin-right button" onclick="ocijeni(); return false;">Ocijeni</button>';
                                        echo "</div>";
                                    }
                                    else {
                                        echo "<p>Proizvođaču $opg ste već dali ocjenu $ocjena!</p>";
                                    }
                                    $rs->close();
                                    $bp->zatvoriDB();
                                }
                                else {
                                    echo "<p>Pojavila se pogreška prilikom potvrde Vašeg identiteta! Za više informacija, obratite se " .
                                         "administratoru sustava.</p>";
                                }
                            }
                            else {
                                echo "<p>Zahvaljujemo Vam se na ocjenjivanju!</p>";
                            }
                            ?>
                        </div>
                    </div>
        ...
        <script type="text/javascript" src="js/ocjena.js?<?php echo filemtime("js/ocjena.js") ?>"></script>
